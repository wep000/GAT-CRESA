...
class MM_GCN(nn.Module):
    ...
    def forward(self, a, v, l, dia_len, qmask, test_label=False):#a(1473,200),v(1473,200),l(1473,200),qmask(77,32,2)
        qmask = torch.cat([qmask[:x, i, :] for i, x in enumerate(dia_len)], dim=0)#qmask(1473,2)对第0个维度拼接起来
        spk_idx = torch.argmax(qmask, dim=-1)#取出utterance下标
        spk_emb_vector = self.speaker_embeddings(spk_idx)#spk_emb_vector(1473,200)
        if self.use_speaker:#default=false
            if 'l' in self.modals:
                l += spk_emb_vector
        if self.use_modal:#default=false
        ...
        adj = self.create_big_adj(a, v, l, dia_len, self.modals, self.modal_weight)#adj(4419,4419)

        if len(self.modals) == 3:
            features_i = torch.cat([a, v, l], dim=0).cuda()#(4419,200)
        ...
        features = self.graph_net(features_i, None, qmask, adj, test_label)#经过k层动态融合模块--->(4419,300)
    ...
    def create_big_adj(self, a, v, l, dia_len, modals, modal_weight=1.0):
        ...
        if len(modals) == 3:
            ...
            return NotImplementedError
        start = 0
        for i in range(len(dia_len)):#对每一个batch
            sub_adjs = []
            for j, x in enumerate(features):#对于每一个模态的特征，
                if j < 0:
                    sub_adj = torch.zeros((dia_len[i], dia_len[i])) + torch.eye(dia_len[i])
                else:
                    sub_adj = torch.zeros((dia_len[i], dia_len[i]))   #(58,58)
                    temp = x[start:start + dia_len[i]]   #(58,200)
                    vec_length = torch.sqrt(torch.sum(temp.mul(temp), dim=1))#(58,200)*(58,200)=>(58,58),然后再第一个维度求和(58,1),然后对每个元素开平方根
                    norm_temp = (temp.permute(1, 0) / vec_length)#norm_temp(200,58)
                    cos_sim_matrix = torch.sum(torch.matmul(norm_temp.unsqueeze(2), norm_temp.unsqueeze(1)), dim=0)
                    '''# seq, seq,(200,58,1)和(200,1,58)作matmul高维矩阵乘法，得到(200,58,58),然后再第0个维度求和得到(,58,58),'''
                    cos_sim_matrix = cos_sim_matrix * 0.99999
                    sim_matrix = 1 - torch.acos(cos_sim_matrix) / np.pi#对于公式3下面一段话
                    sub_adj[:dia_len[i], :dia_len[i]] = sim_matrix
                sub_adjs.append(sub_adj)#第一个元素(58,58)，58是第一个batch中utterance数量
            ...
            for m in range(modal_num):
                for n in range(modal_num):
                    ...
                    if m == n:
                        ...
                        modal1 = features[m][start:start + dia_len[i]]  # length, dim,features{(1473,200),(1473,200),(1473,200)}
