...
def simple_batch_graphify(features, lengths, no_cuda):
    ...
    for j in range(batch_size):
        ...
        '''#lengths长度为32，每个元素为每个对话中Utterance的数量，得到node_features为长度32的list,每个元素长度为(len,200),
        这里len即为lengths中每个元素的长度，也就是不同对话中utterance数不同'''

    node_features = torch.cat(node_features, dim=0)#将所有utterance拼接起来得到(1473,200)
...
class DialogueGNNModel(nn.Module):
    ...
    def __init__(self, base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, n_speakers, max_seq_len, window_past, window_future, n_classes=7,
        ...
        # # ==========改============
        # middle_hidden_size = 200
        # attention_nhead = 8
        # attention_dropout = 0.4
        # self.cross_att = nn.MultiheadAttention(
        #     embed_dim=middle_hidden_size,
        ...
        if self.graph_type in ['GF', 'GF2', 'GDF']:
    ...
    def forward(self, U, qmask, umask, seq_lengths, U_a=None, U_v=None, test_label=False):
        '''#U(77,32,100),qmask(77,32,2),umask(32,77),seq_lengths:长度32的list,U_a(77,32,1582),U_v(77,32,342)'''
        ...
        elif self.base_model == 'LSTM': #==========入口================
            if not self.multi_modal:
                U = self.linear_(U)
                emotions, hidden = self.lstm(U)

                # TODO
                if self.use_crn_speaker: #default=true
                    # (32,21,200) (32,21,9)
                ...
                if 'a' in self.modals:
                    ...
                    emotions_a = U_a        #emotions_a(77,32,200)
                    if self.av_using_lstm:#default=false
                        emotions_a, hidden_a = self.lstm_a(U_a)

                    if self.use_crn_speaker:

                        # (32,21,200) (32,21,9)
                        U_, qmask_ = U_a.transpose(0, 1), qmask.transpose(0, 1)#U_(32,77,200),qmask(32,77,2)
                        U_p_ = torch.zeros(U_.size()[0], U_.size()[1], 200).type(U_a.type())#U_p_(32,77,200)
                        U_parties_ = [torch.zeros_like(U_).type(U_.type()) for _ in range(self.n_speakers)]  # default 2,此时U_parties_是长度为2的list,每个元素都是U_(32,77,200)形状全为0的tensor
                        for b in range(U_.size(0)):  #对每一个batch
                            for p in range(len(U_parties_)): #对每一个speaker
                                index_i = torch.nonzero(qmask_[b][:, p]).squeeze(-1)  #index_i即为每个speaker在每个batch中说话的utterance下标，qmask_(32,77,2),torch.nonzero(qmask_[b][:, p])表示取出某一个batch中某一个speaker的第二个维度中非0的元素下标
                        ...
                        a = self.rnn_parties(U_parties_[p].transpose(0, 1))
                ...
                if 'v' in self.modals:
                    ...
                    if self.use_crn_speaker:   #是否做CRN-speaker_embedding
                        # (32,21,200), (32,21,9)
        ...
        # #======================改============================
        # '''emotions_a(69,8,200),emotions_v(69,8,200),emotions_l(69,8,200)'''
        # emotions_l, _ = self.cross_att(emotions_l,emotions_a,emotions_a)#(69,8,200)
        # emotions_l+=emotions_l
        # emotions_a, _ =self.cross_att(emotions_a,emotions_l,emotions_l)
        # emotions_a+=emotions_a
        # #======================改============================
        if not self.multi_modal:
            ...
            if 'a' in self.modals:
                if self.graph_type == 'relation':
                    ...
                    features_a, edge_index, edge_norm, edge_type, edge_index_lengths = simple_batch_graphify(emotions_a, seq_lengths, self.no_cuda)  #只有features_a不为None，其余全为None
            else:
                features_a = []
            if 'v' in self.modals:
                if self.graph_type == 'relation':
        ...
        if self.graph_type == 'relation':
            ...
            if not self.multi_modal:
                ...
                log_prob = F.log_softmax(self.smax_fc(emotions_feat), 1)
        elif self.graph_type in ['GF', 'GF2', 'GDF']:             #=======================入口========================
            # (1473,300*3)
            emotions_feat = self.graph_model(features_a, features_v, features_l, seq_lengths, qmask, test_label)#emotions_feat(1473,900)
            if test_label:
                index = 15
    ...
    '''#log_prob(1473,6),1473是所有batch中32个对话中所有utterance之和，每个utterance都有对应的情绪标签
       edge_index, edge_norm, edge_type, edge_index_lengths全为None,因为图为非关系型'''
