...
class EncoderLayer(nn.Module):
    def __init__(self,d_model,n_heads):
        ...
        self.d_model = d_model
        self.n_heads = n_heads
        self.enc_self_attn_intra = MultiHeadAttention(self.d_model,self.n_heads)
        self.enc_self_attn_ctx = MultiHeadAttention(self.d_model,self.n_heads)
        self.pos_ffn = PoswiseFeedForwardNet(d_model)

    def forward(self, query,key,context,mask):
        enc_outputs, attn = self.enc_self_attn_intra(query,key,key,mask) # enc_inputs to same Q,K,V
        #intra 注意力以及layerNorm和残差连接后得到的intra信息,接下来要和context做注意力交互
        #和context也再做一次带说话人掩码的注意力交互
        enc_outputs_ctx, attn_ctx = self.enc_self_attn_ctx(enc_outputs, context, context, None)
        enc_outputs_ctx = self.pos_ffn(enc_outputs_ctx) # enc_outputs: [batch_size x len_q x d_model]
        return enc_outputs_ctx, attn_ctx
...
class Encoder(nn.Module):
    ...
    def forward(self, query,key,context,mask): # enc_inputs : [batch_size x source_len]
        enc_outputs = query
        enc_self_attns = []
        for layer in self.layers:
            enc_outputs, enc_self_attn = layer(enc_outputs,key,context,mask)
            enc_self_attns.append(enc_self_attn)
        return enc_outputs, enc_self_attns


class CrossModalTransformer_ctx(nn.Module):
    def __init__(self,d_model,n_heads,n_layers):
        super(CrossModalTransformer_ctx, self).__init__()
        self.encoder = Encoder(d_model,n_heads,n_layers)
    def forward(self, query,key,context,mask):
        enc_outputs, enc_self_attns = self.encoder(query,key,context,mask)
        return enc_outputs,enc_self_attns
